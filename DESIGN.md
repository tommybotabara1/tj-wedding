...
- **Opening loader** — The first thing shown, before the access gate: a ~2.3s plane-takeoff sequence (a sage trail draws as the plane lifts off along an arc, "Soaring with TJ" rises in script). Reduced-motion → a calm static fade. Flow: loader → access gate → invitation.
- **In-Flight Feature** — A "Now Showing" section near the top presenting the prenup film, framed like a seat-back cinema screen (kraft bezel, rounded). The `savethedate.webp` still is the poster; tapping play lazy-loads the Google Drive player inline. (This replaced the old save-the-date card in Journey.)
- **Send a Gift** — A button in the gifts section opens a native `<dialog>` with five payment tiles (BPI, BDO, UnionBank, PNB, GCash), each a framed QR + label. QR images live in `docs/images/qr/`; missing ones show a dashed "Add QR" placeholder and auto-fill when the file is added.
